...
def draw_leaderboard(leader_names, leader_scores, high_scorer, turtle_object, player_score):
  ...
  while leader_index < len(leader_names):
    ...
    turtle_object.penup()
    turtle_object.goto(-120,int(turtle_object.ycor())-50)
    turtle_object.down()
    leader_index = leader_index + 1

  # Display message about player making/not making leaderboard based on high_scorer
  if (high_scorer):
    turtle_object.write("Congratulations!", font=("Arial", 10, "normal"))
    turtle_object.right(90)
    turtle_object.forward(20)
    turtle_object.left(90)
    turtle_object.write("You made the leaderboard!", font=("Arial", 10, "normal"))
  else:
